...
describe('TitaniumCircuitBreaker', () => {
  ...
  it('shouldBlock returns false if no entry exists', async () => {
    ...
    const result = await TitaniumCircuitBreaker.shouldBlock(env, CircuitService.WHATSAPP);
  ...
  it('recordSuccess updates status to closed', async () => {
    ...
    expect(dbMock.prepare).toHaveBeenCalledWith(expect.stringContaining("status = 'closed'"));
  });
  ...
  it('recordFailure increments failure count and trips if >= 3', async () => {
    ...
    await TitaniumCircuitBreaker.recordFailure(env, CircuitService.WHATSAPP, 500);
    expect(dbMock.prepare).toHaveBeenCalledWith(expect.stringContaining("CASE WHEN status = 'open' OR failure_count + 1 >= 3 THEN ? ELSE opened_at END"));

    await TitaniumCircuitBreaker.recordFailure(env, CircuitService.WHATSAPP);
    expect(dbMock.prepare).toHaveBeenCalledWith(expect.stringContaining("failure_count = failure_count + 1"));
  });

  it('trip sets status to open and failure count to 3', async () => {
    await TitaniumCircuitBreaker.trip(env, CircuitService.WHATSAPP);
    expect(dbMock.prepare).toHaveBeenCalledWith(expect.stringContaining("'open', ?, 3"));
  });
});
