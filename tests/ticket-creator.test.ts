...
import { TicketCreator, calculateEndTime } from '../shared/services/ticket-creator';
...
describe('TicketCreator', () => {
  ...
  it('createTicket throws error if slot occupied', async () => {
    const session: any = {
      ...
      const creator = new TicketCreator(dbMock);
      await expect(creator.createTicket(session)).rejects.toThrow('Slot already occupied');
  });
