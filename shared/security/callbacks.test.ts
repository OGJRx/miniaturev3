...
describe("parseCallback expiration", () => {
  ...
  it("should mark a callback older than 1440 minutes as expired", async () => {
    const start = new Date("2024-05-20T12:00:00Z");
    vi.setSystemTime(start);

    const callback = await buildCallback("reg_brand", "toyota", secret);

    // Advance time by 1441 minutes
    const later = new Date(start.getTime() + 1441 * 60 * 1000);
  ...
  it("should mark a callback exactly 1440 minutes old as NOT expired", async () => {
    const start = new Date("2024-05-20T12:00:00Z");
    vi.setSystemTime(start);

    const callback = await buildCallback("reg_brand", "toyota", secret);

    // Advance time by exactly 1440 minutes
    const later = new Date(start.getTime() + 1440 * 60 * 1000);
    vi.setSystemTime(later);
